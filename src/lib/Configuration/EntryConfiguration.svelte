<script lang="ts">
    ...
    let { entry = $bindable(), onnewtimer, ondelete, oncomplete, index } = $props();
...
<div class={`flex flex-row p-4 gap-4 rounded-lg shadow-lg mb-4 ${entry.completed ? 'bg-green-900/35' : 'bg-gray-700'}`}>
